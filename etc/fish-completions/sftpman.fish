...
complete --command sftpman --condition "__fish_seen_subcommand_from create update" --long-option mount_opt --description "Mount options" --arguments "(sshfs --help 2>&1 | grep '\-o' | cut --delimiter '-' --fields=2 | cut --delimiter ' ' --fields=2 | grep --word-regexp --extended-regexp --only-matching '^[a-z_]+')"
